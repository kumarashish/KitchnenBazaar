...
public class WebApiCall {
    ...
    public void postDataWithJSON(String url,String json ,final WebApiResponseCallback callback) {
        client.newBuilder().connectTimeout(60000, TimeUnit.MILLISECONDS).readTimeout(60000, TimeUnit.MILLISECONDS).build();
        final Request.Builder builder = new Request.Builder();
        RequestBody reqBody = RequestBody.create(JSON,json);
        final Request request =builder.url(url).post(reqBody).build();
        client.newCall(request).enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                callback.onError(e.fillInStackTrace().toString());
    ...
    public void postDataWithHeader(String url,String[]key,String[]value ,final WebApiResponseCallback callback) {
        client.newBuilder().connectTimeout(60000, TimeUnit.MILLISECONDS).readTimeout(60000, TimeUnit.MILLISECONDS).build();
        final Request.Builder builder = new Request.Builder();
        ...
        RequestBody reqBody = RequestBody.create(JSON,new JSONObject().toString());
        final Request request =builder.url(url).post(reqBody).build();
        client.newCall(request).enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
